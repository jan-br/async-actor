...
use crate::util::{format_injectable_struct_instantiation, format_generic_constraints, format_generic_definition, format_generics_as_tuple, format_handle_name, format_instantiation_data_name, format_name, find_non_injectable_non_default_fields};
...
fn create_assisted_instantiable_implementation(original: &ItemStruct) -> Result<ItemImpl> {
  ...
  let instantiation = format_injectable_struct_instantiation(&original, &quote! {Self}, false, Some(non_injectable_non_default_fields.clone()));
  ...
  syn::parse2(quote! {
    ...
    impl async_actor::inject::assisted_inject::AssistedInstantiable<#instantiation_data_name #generic_definition> for #original_name #generic_constraints {
      async fn instantiate(injector: async_actor::inject::InjectorHandle, data: #instantiation_data_name #generic_definition) -> #original_name{
        let #instantiation_data_name #generic_definition { #(#non_injectable_non_default_fields,)* .. } = data;
